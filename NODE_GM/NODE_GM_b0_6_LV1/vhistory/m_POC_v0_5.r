...
for(i in 1:ncol(Y))
{
  ...
  plot(t,Y[,i])
	...
	#
	##
	plot(t_,ddt.Ybar_o_MaP[,i],type="l",col="red",lty=2)
	...
	lines(t_true/20*40+1,ddt.Ybar_true[,i]/2,lty=2,type="l")
}
par(mfrow=c(1,1))
...
for(i in 1:ncol(Y))
{
	for(j in 1:3)
	{
		...
		x = density(tmp)$x
		y = density(tmp)$y
		pd = 1/sqrt(2*pi*sd2_o^2)*exp(-(x^2)/(2*sd2_o^2))
