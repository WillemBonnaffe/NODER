...
# t      - vector - time steps
# Y      - vector - observations 
# N_p    - int    - number of parameters in observation model 
...
fit.model_o = function(t,Y,N_p,sd_1,sd_2,N_e,logMar=F)
{
	## fit observation model
	ensemble = NULL
	for(k in 1:N_e)
	{
	    Omega_0 = rnorm(N_p,0,sd_2)
		if (logMar==F)
		{
...
# sd_2   - float  - standard deviation of prior
# N_e    - int    - number of samples to take
fit.model_p = function(X,Y,N_p,sd_1,sd_2,N_e)
{
	## fit model
	ensemble= NULL 
	for(k in 1:N_e)    
