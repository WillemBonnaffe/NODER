...
## 29-03-2022 - created v0_23
##            - implemented muliple fold cross validation
...
## parameters
N_o    	 = 20      	       # number of ensemble elements
...
for(i in 1:ncol(rhat_o))
{
	...
	for(k in 1:length(sd2_p_Vect))
	{
		...
		folds     = list(c(0,2/3),c(0.5/3,2.5/3),c(1/3,3/3))
		# folds     = list(c(0,2/4),c(1/4,3/4),c(2/4,4/4))
		nfolds    = length(folds)
		crossVal_ = NULL
		ensembleList_ = list()
		for(u in 1:nfolds)
		{
			...
			## split training/test
			# s = sample(1:length(Y_),round(2/3*length(Y_))) 
			...
			## model performance
			logLik_l      = apply(ensemble_p,1,function(x)logLik_p(X_l,Y_l,x,sd1_p))
			logLik_t      = apply(ensemble_p,1,function(x)logLik_p(X_t,Y_t,x,sd1_p))
					
			## cross validation matrix
			crossVal_  = rbind(crossVal_,cbind(logLik_l,logLik_t))
			
			## store
			ensembleList_[[u]] = ensemble_p
		}

		## store 
		E.crossVal_       = apply(crossVal_,2,mean)
		sd.crossVal_      = apply(crossVal_,2,sd)
		crossVal          = rbind(crossVal,c(sd2_p,E.crossVal_,sd.crossVal_))
		ensembleList[[k]] = ensembleList_
	}
	...
	y = crossVal[,c(2,3)]
	sd.y = crossVal[,c(4,5)]
	plot(x,rep(0,length(x)),ylim=c(min(y-sd.y)-0.1*abs(min(y-sd.y)),max(y+sd.y)+0.1*abs(max(y+sd.y))),type="l",lty=3,xlab="sd",ylab="error")
	#
	...
	## best regularisation
	k = which.max(crossVal[,3])
	ensemble_p = rbind(ensembleList[[k]][[1]],ensembleList[[k]][[2]],ensembleList[[k]][[3]])
