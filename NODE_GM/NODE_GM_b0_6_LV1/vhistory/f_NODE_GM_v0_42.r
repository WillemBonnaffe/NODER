...
.plot.diamond = function(df,y_lim=NULL,colVect=NULL,y_lab="",main="")
{
  ...
  ## plot
  plot(rep(1,n_row),df[,1],xlim=x_lim,ylim=y_lim,cex=0,xlab="",ylab=y_lab,xaxt="n",yaxt="n",main=main)
  axis(side=1,at=x,labels=x,tick=F,line=0)
  axis(side=2,at=seq(-1.5,1.5,0.1),labels=seq(-1.5,1.5,0.1),tick=F,line=0)
  for(j in 1:n_col)
  {
    ## coordinates
    x_ = rep(j,n_row)
...
# args:
# df - matrix - matrix containing variables to be plotted by columns
.plot.Hdiamond = function(df,y_lim=NULL,colVect=NULL,y_lab="",main="",labels=NULL)
{
  ## reverse columns
  df = df[,ncol(df):1]
  ...
  # plot(df[,1],rep(1,n_row),xlim=y_lim,ylim=x_lim,cex=0,xlab="",ylab=y_lab,xaxt="n",yaxt="n",main=main)
  # axis(side=1,at=seq(-1.5,1.5,0.1),labels=seq(-1.5,1.5,0.1),tick=F,line=0)
  axis(side=2,at=x,labels=labels,tick=F,line=0,las=1)
  for(j in 1:n_col)
  {
    ## coordinates
    x_ = rep(j,n_row)
...
## plot.NODEGM_o
## goal:
plot.model_o = function(t,t_,Y,results_o,col="red")
{
...
## plot.model_p
## goal: 
plot.model_p = function(t,t_,Y,results_p,col="red")
{
...
fit.NODEGM_o = function(t,t_,Y,W_o,sd1_o,sd2_o,N_o,log=F,logMar=F)
{
	...
	for(i in 1:ncol(Y))
	{
		Omega_o_chain = fit.model_o(t,Y[,i],W_o,sd1_o,sd2_o,N_o,log,logMar)
		results_o[[i]]   = predict.model_o(t_,Omega_o_chain,log)
	}
	return(results_o)
}
...
##
plot.NODEGM_o = function(t,t_,Y,results_o,col="red")
{
	layout(matrix(1:(ncol(Y)*2),byrow=F,nrow=2))
	for (i in 1:ncol(Y)) plot.model_o(t,t_,Y[,i],model_o[[i]],col=col[i])
}

##
plot.NODEGM_p = function(t,t_,Y,results_p,col="red")
{
	layout(matrix(1:(ncol(Y)*3),byrow=F,nrow=3))
	for (i in 1:ncol(Y)) plot.model_p(t,t_,Y[,i],model_p[[i]],col=col[i])
}
...
{
	N = length(outputList)
	layout(matrix(1:(N*2),byrow=F,nrow=2))
	par(cex.lab=1,cex.axis=1,cex.main=1,mar=c(1,1,1,1)*3,oma=c(1,10,1,1))
	layout(matrix(1:(N*2),byrow=T,ncol=2))
	mainVect = matrix(c("a.","b.","c.","d.","e.","f.","g.","h.","i.","j.")[1:(N*2)],byrow=T,ncol=2)
	labels_   = rep("",N)
	#
	for(i in 1:N)
	{
		y = outputList[[i]]$prop.tot.Geber_p
		.plot.Hdiamond(y,labels=labels,main= if (i == 1) "Total contributions"  else "" )
		legend("topright",legend=mainVect[i,1],bty="n",cex=1)
		#
		y = outputList[[i]]$mean.ddx.Yhat_p
		.plot.Hdiamond(y,labels=labels_,main= if (i == 1) "Mean effects"  else "" )
