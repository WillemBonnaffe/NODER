...
for(i in 1:ncol(Y))
{
	for(j in 1:3)
	{
		tmp = Omega_o[[i]][1:W_o+W_o*(j-1)]
		x = density(tmp)$x
		y = density(tmp)$y
		plot(x,1/sqrt(2*pi*sd2_o^2)*exp(-(x^2)/(2*sd2_o^2)),type="l",lty=2)
		lines(x,y)
	}
}
par(mfrow=c(1,1))
...
for (i in 1:ncol(Y))
{
	...
	Omega_p_      = argmax.logPost_p(std.Ybar_o,1/Ybar_o[,i]*ddt.Ybar_o[,i],Omega_p_,sd1_p,sd2_p)
	Omega_p[[i]]  = Omega_p_
}
...
for(i in 1:ncol(Y))
{
	ddt.Ybar_p_     = ddt.f_p.eval(std.Ybar_o,Omega_p[[i]])
	ddt.Ybar_p      = cbind(ddt.Ybar_p,ddt.Ybar_p_)
	ddx.ddt.Ybar_p_ = 1/sd_*t(ddx.ddt.f_p.eval(std.Ybar_o,Omega_p[[i]]))
...
for(i in 1:ncol(Y))
{
	...
	lines(t_,ddt.Ybar_p[,i],type="l",col="red")

	##
	plot(t_,ddx.ddt.Ybar_p[,1+(i-1)*2],ylim=c(-2,2),col="white")
	for(j in 1:ncol(Y))
	{
		lines(t_,ddx.ddt.Ybar_p[,j+(i-1)*2],type="l",col=j)
