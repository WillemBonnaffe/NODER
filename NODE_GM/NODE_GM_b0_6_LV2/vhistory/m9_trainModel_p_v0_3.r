...
for(i in 1:N)
{
	...
	{   
	    ...
	    # Omega_f   = argmax.logPost(X_,Y_[,i],f_p.eval,ddOmega.f_p.eval,Omega_0,sd1_p,sd2_p[i])
	    Omega_f   = argmax.logMar(X_,Y_[,i],f_p.eval,ddOmega.f_p.eval,Omega_0)
	    Omega_p_i = rbind(Omega_p_i,Omega_f)

		## update
	    # logPost_0 = logPost(X_,Y_[,i],f_p.eval,Omega_0,sd1_p,sd2_p[i])
	    # logPost_f = logPost(X_,Y_[,i],f_p.eval,Omega_f,sd1_p,sd2_p[i])
        logPost_0 = logMar(X_,Y_[,i],f_p.eval,Omega_0)
	    logPost_f = logMar(X_,Y_[,i],f_p.eval,Omega_f)
	    message(paste(k,"/",K_p,"\t",
