...
f_p.eval           = function(X,Omega) apply(X,1,function(x) f_p(x,Omega))
ddx.f_p.eval       = function(X,Omega) apply(X,1,function(x) ddx.f_p(x,Omega))
ddOmega_p.f_p.eval = function(X,Omega) apply(X,1,function(x) ddOmega_p.f_p(x,Omega))
...
{
  ## predict
  ddt.Ybar_p = f_p.eval(Ybar_o,Omega_p)
...
ddOmega_p.logPost_p = function(Ybar_o,ddt.Ybar_o,Omega_p,sd1_p,sd2_p)
{
  ...
  ## derivative of residuals
  res_p           = ddt.Ybar_o - ddt.Ybar_p
  ddOmega_p.res_p = - ddOmega_p.f_p.eval(Ybar_o,Omega_p)
...
logMar_p = function(Ybar_o,ddt.Ybar_o,Omega_p,sd1_p,sd2_p)
{
  ## predict
  ddt.Ybar_p = f_p.eval(Ybar_o,Omega_p)
...
ddOmega_p.logMar_p = function(Ybar_o,ddt.Ybar_o,Omega_p,sd1_p,sd2_p)
{
	## predict
	ddt.Ybar_p = f_p.eval(Ybar_o,Omega_p)
...
fit.model_o = function(t,Y,W_o,sd1_o,sd2_o,N_e,log=F,logMar=T)
{
	## standardise time steps
	t       = 1:length(t)                               # vector of time steps

	## standardise/transform data
	if (log==T) Y = log(Y)    	                        # log facilitates fitting
	mean_y = mean(Y)
	sd_y   = sd(Y)
	Y     = (Y-mean_y)/sd_y
...
## goal:
## args:
predict.model_o = function(t_,Y,Omega_o_chain,log=F)
{
	## standardise time steps
	t_real  = t_
	dt_real = diff(t_real[1:2])
	t_       = 1:length(t_)                               # vector of time steps
	...
	t_ = (t_-min(t_))/(max(t_)-min(t_))*(max(t_real)-min(t_real))+min(t_real)
	...
	## terminate
	results_o = list(t_,MaP.Yhat_o,E.Yhat_o,q05.Yhat_o,q95.Yhat_o,MaP.ddt.Yhat_o,E.ddt.Yhat_o,q05.ddt.Yhat_o,q95.ddt.Yhat_o)
	names(results_o) = c("t_","MaP.Yhat_o","E.Yhat_o","q05.Yhat_o","q95.Yhat_o","MaP.ddt.Yhat_o","E.ddt.Yhat_o","q05.ddt.Yhat_o","q95.ddt.Yhat_o")
	return(results_o)
}
...
## output:
## Omega_p    - vector - vector of parameters of the fitted process model
predict.model_p = function(X,ddt.X,Y,Omega_p_chain)
{
	## standardise predictive variables
	...
	## evaluate ensemble
	Yhat_p         = t(apply(Omega_p_chain[,-1],1,function(x) f_p.eval(X_,x)))
	E.Yhat_p       = apply(Yhat_p,2,mean)
	q05.Yhat_p     = apply(Yhat_p,2,quantile,p=0.05)
	q95.Yhat_p     = apply(Yhat_p,2,quantile,p=0.95)
	ddx.Yhat_p     = t(apply(Omega_p_chain[,-1],1,function(x) ddx.f_p.eval(X_,x)))
	E.ddx.Yhat_p   = t(matrix(apply(ddx.Yhat_p,2,mean),ncol=length(t_)))
	q05.ddx.Yhat_p = t(matrix(apply(ddx.Yhat_p,2,quantile,p=0.05),ncol=length(t_)))
	q95.ddx.Yhat_p = t(matrix(apply(ddx.Yhat_p,2,quantile,p=0.95),ncol=length(t_)))
	...
	## store
	pred_p        = list(E.Yhat_p,q05.Yhat_p,q95.Yhat_p,E.ddx.Yhat_p,q05.ddx.Yhat_p,q95.ddx.Yhat_p,E.Geber_p,q05.Geber_p,q95.Geber_p)
	names(pred_p) = c("E.Yhat_p","q05.Yhat_p","q95.Yhat_p","E.ddx.Yhat_p","q05.ddx.Yhat_p","q95.ddx.Yhat_p","E.Geber_p","q05.Geber_p","q95.Geber_p")
	return(pred_p)
}
