...
{	
	...
	return(SLP(x,Omega[,1],lin) + SLP(x,Omega[,2],sigmo))
	# return(SLP(x,Omega[,1],lin) + SLP(x,Omega[,2],pol2))
}
...
{
	Omega = matrix(Omega,ncol=2)
	return(c(ddOmega.SLP(x,Omega[,1],lin,ddx.lin),ddOmega.SLP(x,Omega[,2],sigmo,ddx.sigmo)))
	# return(c(ddOmega.SLP(x,Omega[,1],lin,ddx.lin),ddOmega.SLP(x,Omega[,2],pol2,ddx.pol2)))
}
...
{
	Omega = matrix(Omega,ncol=2)
	return(ddx.SLP(x,Omega[,1],lin,ddx.lin) + ddx.SLP(x,Omega[,2],sigmo,ddx.sigmo))
	# return(ddx.SLP(x,Omega[,1],lin,ddx.lin) + ddx.SLP(x,Omega[,2],pol,ddx.pol2))
}
...
## sample.model_p ##
## goal: sample process model using DEMC
## out:  returns list containing metadata regarding the scaling of variables and an ensemble of parameter vectors 
# X      - matrix - explanatory variables by columns (e.g. interpolated state variables)
# Y      - vector - response variable (e.g. temporal derivative of an interpolated state variable)
# W      - int    - number of hidden nodes in the process model
...
# log    - bool   - whether to log the response variable (e.g. if its support is positive)
# logMar - bool   - whether to optimise the log posterior or marginal
sample.model_p = function(X,Y,W,sd_1,sd_2,N_e,log=F,logMar=F)
{
	## standardise predictive variables
	X_     = X
	if (log == T) X_ = log(X_)
	...
	## fit model
	Omega_0 = rnorm(W*(2+ncol(X_))*2,0,sd_2)
	dTarget = function(x) logPost_p(X_,Y_,x,sd_1,sd_2)
	ensemble = DEMCpp(list("dTarget" = dTarget,
						   "Theta_0" = Omega_0, 
						   "epsilon" = 0.001, 
						   "nIt"     = N_e))$chainList
...
# ...
# legend - vector - index
plot.model_p = function(t_,Y,results_p,col="red",xlab=c("Time","Time","Time"),ylab=c("Dynamics","Effects","Contributions"),index=NULL)
{
	## initiate
...
# ...
plot.NODEGM_p = function(t_,Y,results,col="red",xlab=c("","","Time"),ylab=c("Dynamics","Effects","Contributions"),index=NULL)
{
	## initiate
	par(mar=c(4,4,0,0),oma=c(1,1,1,1))
	...
	for (i in 1:ncol(Y)) 
	{
		plot.model_p(t_,Y[,i],results[[i]],col[i],xlab,if (i == 1) ylab else rep("",3),index[1:3+(i-1)*3])
	}
	#
	par(mfrow=c(1,1))
}
