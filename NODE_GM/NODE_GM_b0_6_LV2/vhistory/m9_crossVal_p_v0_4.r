	...
	for(k in 1:length(crossValParVect)) # for each regularisation param
	{
		...
		for(u in 1:length(folds)) # for each fold in the data
		{
			...
			{   
			    ...
			    ## fit
                N_p          = 2 * crossValParVect[k] * (2+N)
			    Omega_0      = rnorm(N_p,0,0.1)
                Yhat         = function(X,Omega) f_p.eval(X,Omega)
                ddOmega.Yhat = function(X,Omega) ddOmega.f_p.eval(X,Omega)
			    Omega_f      = argmax.logMarPost(X_l,Y_l[,i],Yhat,ddOmega.Yhat,Omega_0,1/crossValParVect[k])
			    Omega_p_iku  = rbind(Omega_p_iku,Omega_f)

				## update
				logMarPost_0 = logMarPost(X_l,Y_l[,i],Yhat,Omega_0,1/crossValParVect[k])
				logMarPost_f = logMarPost(X_l,Y_l[,i],Yhat,Omega_f,1/crossValParVect[k])
				# message(paste(m,"/",K_p,"\t",
