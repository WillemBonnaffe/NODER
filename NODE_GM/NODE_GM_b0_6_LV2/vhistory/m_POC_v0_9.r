...
# sd2_o  	= c(rep(0.2,W_o),rep(0.2,W_o),rep(0.2,W_o))	 		# sd in priors of observation model
...
# 	pred_o[[i]]    = predict.model_o(t_,Y,Omega_o_chain,log=T)
...
# 	Omega_o_MaP = Omega_o_chain[which.max(Omega_o_chain[,1]),-1]
...
# 	pred_p[[i]]   = predict.model_p(X,Y,Omega_p_chain)
# 	Geber_p[[i]]  = Geber.model_p(X,ddt.X,Y,Omega_p_chain)
