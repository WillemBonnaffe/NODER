...
eval.model_p = function(Ybar_o,Omega_p)
{
	...
	prop.C_p       = t(apply(C_p,1,function(x) x^2/sum(x^2)))
...
plot.NODEGM = function(TS,results)
{
	attach(results)
	t = TS[,1]
	N = ncol(TS) - 1
	...
	for(i in 1:N)
	{
		par(mfrow=c(5,1),mar=c(3,5,1,1),cex.lab=1.5)
		#
		## interpolation
		plot(t_,rep(0,length(t_)),ylim=c(-2,2),type="l",lty=2,ylab="State")
		points(t,Y[,i])
		...
		#
		## dynamics
		plot(t_,rep(0,length(t_)),ylim=c(-1,1),type="l",lty=2,ylab="Dynamics")
		...
		#
		## Geber 
		plot(t_,rep(0,length(t_)),ylim=c(-1,1),type="l",lty=2,ylab="Contribution")
