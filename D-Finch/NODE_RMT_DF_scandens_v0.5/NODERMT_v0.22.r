...
# 25-05-2021 - created version 0.22
#            - correct the calculation of the contribution matrix to no give weight to a variable when all contributions are small
#            - still need to correct not taking the mean of the mean, but the sum of square

## initiate model
fit.NODE = function(N_e = 30, W_o = 10, W_p = 10, sd1_o = 0.01, sd2_o = 0.1, sd1_p = 0.01, sd2_p = 0.1, alpha = 10)
{
	...
	for(i in 1:N)
	{
	  ...
	  C_mean_meansq = apply(C_mean^2,2,sum) 
	  ...
	  # CMat = rbind(CMat,apply(Ctot_mean,2,sum)) # v0.21 corrected in version 0.22
	  CMat = rbind(CMat,C_mean_meansq) # v0.22
	  r2Vect = c(r2Vect,r2)
