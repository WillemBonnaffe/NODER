...
## store elements
bifurcationList = c(bifurcationList,list(list(lambda,r2,EMat,CMat))) 
...
resTab = NULL
for(i in 1:length(bifurcationList)){resTab = rbind(resTab,unlist(bifurcationList[[i]][c(1,2)]))}
...
for(i in 1:N)
{
	...
	{
		lambdaVect = c(lambdaVect,bifurcationList[[j]][[1]])
		effectsTab = rbind(effectsTab,bifurcationList[[j]][[3]][i,])
	}
	plot(lambdaVect,effectsTab[,1],ylim=c(-3,3),cex=0)
	for(k in 1:N)
	{
		lines(lambdaVect,effectsTab[,k],col=rainbow(N)[k])
	}
}
...
{
	lambdaVect = NULL
	contributionTab = NULL
	for(j in 1:length(bifurcationList))
	{
		lambdaVect = c(lambdaVect,bifurcationList[[j]][[1]])
		contributionTab = rbind(contributionTab,bifurcationList[[j]][[4]][i,])
	}
	plot(lambdaVect,contributionTab[,1],ylim=c(0,1),cex=0)
	for(k in 1:N)
	{
		lines(lambdaVect,contributionTab[,k],col=rainbow(N)[k])
	}
}
