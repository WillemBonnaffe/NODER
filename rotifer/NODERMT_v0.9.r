...
## format data (first column should be time)
TS[,1] = TS[,1] - min(TS[,1])

## transform real variables
tmp = c("R","G","B")
for(i in 2:ncol(TS)) TS[which(TS[,i]<0.001),i] = 0.005
TS[,tmp] = log(TS[,tmp])
...
## visualise 2D phase space
x = cbind(0,0,seq(-2,2,0.1))
y = t(ddx.ddt.f_p.eval(x,Omega_p))
plot(x[,3],y[,1],type="l",ylim=c(-2,2),col="white")
for(i in 1:3){lines(x[,3],y[,i],col=rainbow(N)[i])}
...
for(i in 1:N)
{
	# res_o = Y - Ybar_o[s,]
	# barplot(res_o[,i])
  res_p = ddt.Ybar_o - ddt.Ybar_p_q0.50
	barplot(res_p[,i])
