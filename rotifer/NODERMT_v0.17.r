...
## TS
TS = read.table("TS_3.csv",sep=";",header=T)
...
# }
# 
# logPrior_o = function(t,Y,Omega_o)
# {
#   ## prior
#   logPrior_o = - sum(Omega_o^2)/(sd2_o^2)
# 
...
for(i in 1:N)
{
    ...
    ## store
    Ybar_o = cbind(Ybar_o,Ybar_o_q0.50)
    ddt.Ybar_o = cbind(ddt.Ybar_o,ddt.Ybar_o_q0.50)
}
#
dev.off()
...
for(i in 1:N)
{
  ...
  EMat = rbind(EMat,Emean_q0.50)
  CMat = rbind(CMat,apply(Ctot_q0.50,2,sum))

  par(mfrow=c(4,1))
  #
  ## plot dynamics
  plot(t_,ddt.Ybar_o[,i],type="l")
  polygon(c(t_,rev(t_)),c(ddt.Ybar_p_q0.05,rev(ddt.Ybar_p_q0.95)),border=NA,col=rainbow(N,alpha=0.2)[i])
  lines(t_,ddt.Ybar_p_q0.50,type="l",col=rainbow(N)[i])
  #
  ## plot effects
  plot(t_,ddx.ddt.Ybar_p_q0.50[,1],ylim=c(-3,3),cex=0)
  for(j in 1:N)
  {
    polygon(c(t_,rev(t_)),c(ddx.ddt.Ybar_p_q0.05[,j],rev(ddx.ddt.Ybar_p_q0.95[,j])),border=NA,col=rainbow(N,alpha=0.2)[j])
    lines(t_,ddx.ddt.Ybar_p_q0.50[,j],type="l",col=rainbow(N)[j])
  }
  lines(t_,rep(0,n_),lty=2)
  #
  ## plot contributions
  plot(t_,C_q0.50[,1],ylim=c(-3,3),cex=0)
  for(j in 1:N)
  {
    polygon(c(t_,rev(t_)),c(C_q0.05[,j],rev(C_q0.95[,j])),border=NA,col=rainbow(N,alpha=0.2)[j])
    lines(t_,C_q0.50[,j],type="l",col=rainbow(N)[j])
  }
  lines(t_,rep(0,n_),lty=2)
  #
  ## plot contributions
...
#   X_q0.50 = Ctot_q0.50
#   X_q0.95 = Ctot_q0.95
#   plot(t_,X_q0.50[,1],ylim=c(-3,3),cex=0)
#   for(j in 1:N)
#   {
#     # polygon(c(t_,rev(t_)),c(X_q0.05[,j],rev(X_q0.95[,j])),border=NA,col=rainbow(N,alpha=0.2)[j])
#     lines(t_,X_q0.50[,j],type="l",col=rainbow(N)[j])
