...
{	
	...
	ddOmega1. = pi*Omega[,2]*cos(pi*(x*Omega[,2] + Omega[,3]))
	ddOmega2. = Omega[,1]*(pi*cos(pi*(x*Omega[,2] + Omega[,3])) - pi*Omega[,2]*x*sin(pi*(x*Omega[,2] + Omega[,3])))
	ddOmega3. = -Omega[,1]*(pi*Omega[,2]*1*sin(pi*(x*Omega[,2] + Omega[,3])))
	return(c(ddOmega1.,ddOmega2.,ddOmega3.))
}
...
ddt.f_p = function(x,Omega) # not safe for work
{	
...
# ddOmega_p.ddt.f_p = function(x,Omega) # not safe for work
# {
# 	Omega = matrix(Omega,nrow=W_p)
# 	ddOmega_p1 = f_sigma(Omega[,2] + Omega[,-c(1:2)]%*%x)
# 	ddOmega_p2 = Omega[,1]*1*ddu.f_sigma(Omega[,2] + Omega[,-c(1:2)]%*%x)
# 	ddOmega_p3 = Omega[,1]**ddu.f_sigma(Omega[,2] + Omega[,-c(1:2)]%*%x)
# }
# ddOmega_o.ddt.d_p = function(x,Omega) # not safe for work
